...
export function generateStaticParams() {
  return getProjectSlugsByDomain('engineering').map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/engineering/${project.slug}` },
  };
}
...
export default async function EngineeringCaseStudy({ params }: RouteParams) {
  const { slug } = await params;
  const project = findProjectBySlug(slug);
  if (!project || !project.domains.includes('engineering')) notFound();
    ...
    <article className='bg-slate-950 text-slate-100 pt-28 md:pt-32 pb-20 md:pb-24'>
      <div className='max-w-3xl mx-auto px-6 md:px-10'>
        ...
        <header className='mt-6'>
          ...
          {project.tagline && (
            <p className='mt-4 text-lg text-cyan-300/90 leading-relaxed'>
              {project.tagline}
            </p>
          )}
        </header>
        ...
        <section className='mt-12 rounded-2xl border border-slate-800 bg-slate-900/40 p-6 text-sm text-slate-400'>
          ...
          <p className='mt-2'>
            Architecture, key product decisions, and screenshots will live
            here. The GitHub repository contains the source.
          </p>
        </section>
      </div>
    </article>
